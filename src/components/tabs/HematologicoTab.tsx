...
const HematologicoTab = () => {
  ...
  // ROTEM
  const [extemCt, setExtemCt] = usePersistedState("hemato_extemCt", "");
  const [fibtemMcf, setFibtemMcf] = usePersistedState("hemato_fibtemMcf", "");
  const [extemMcf, setExtemMcf] = usePersistedState("hemato_extemMcf", "");
  const [extemMl, setExtemMl] = usePersistedState("hemato_extemMl", "");
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ROTEM */}
      <CollapsibleSection title="Teste Viscoelástico (ROTEM / TEG)"
        info={<InfoTooltip interpretation="Avaliação funcional da coagulação para hemorragia major. Protocolo de transfusão guiado." />}>
        ...
        <div className="mb-4 bg-muted/20 border border-border rounded-lg p-3">
          <p className="text-[10px] font-semibold text-foreground mb-3 text-center">Interpretação Visual</p>
          <div className="flex justify-around items-end">
            <RotemTrace type="normal" label="Normal" />
            <RotemTrace type="prolonged_ct" label="Défice Fatores (CT longo)" />
            <RotemTrace type="low_mcf" label="Défice Plaquetas/Fib (MCF bx)" />
            <RotemTrace type="hyperlysis" label="Hiperfibrinólise (ML alto)" />
          </div>
        </div>

        {(() => {
          const ct = parseFloat(extemCt);
          const fMcf = parseFloat(fibtemMcf);
          const eMcf = parseFloat(extemMcf);
          const ml = parseFloat(extemMl);
          
          const msgs = [];
          
          if (ml > 15) {
            msgs.push({ text: `EXTEM ML ${ml}% (>15%) — Hiperfibrinólise: Dar Ácido Tranexâmico (TXA)`, level: "danger" });
          }
          if (fMcf > 0 && fMcf < 9) {
            msgs.push({ text: `FIBTEM MCF ${fMcf}mm (<9mm) — Défice de fibrinogénio: Dar Concentrado de Fibrinogénio ou Crioprecipitado`, level: "danger" });
          }
          if (eMcf > 0 && eMcf < 50 && fMcf >= 9) {
            msgs.push({ text: `EXTEM MCF ${eMcf}mm (<50) c/ FIBTEM normal — Défice plaquetário: Transfundir Plaquetas`, level: "warning" });
          }
          if (ct > 80 && fMcf >= 9) {
            msgs.push({ text: `EXTEM CT ${ct}s (>80) c/ FIBTEM normal — Défice de fatores: Dar Complexo Protrombínico (PCC) ou Plasma (PFC)`, level: "warning" });
          }

          if (msgs.length === 0 && (ct > 0 || fMcf > 0 || eMcf > 0 || ml > 0)) {
            return <AlertBanner text="Parâmetros dentro do alvo ou sem indicações hemorrágicas claras no protocolo base." level="info" />;
          }

          return msgs.map((m, i) => (
             <AlertBanner key={i} text={m.text} level={m.level as any} />
          ));
        })()}
      </CollapsibleSection>
      ...
      <CollapsibleSection title="Enoxaparina — Ajuste Renal"
        ...
        <div className="mt-2">
          <Interpretation text={`Dose baseada no peso e função renal. ${onRRT || (clCr !== null && clCr < 30) ? "Na Lesão Renal ou TRS, HNF pode ser preferida por ter menor acumulação." : ""} `} />
        </div>
